...
function buildSofa(cfg) {
  ...
  const mat = makeStd({ color: 0x4a3a3a, roughness: 0.85, metalness: 0.0 });
  const accent = makeStd({ color: 0x2a2a35, roughness: 0.6, metalness: 0.2 });
  const cushion = makeStd({ color: 0x5a4a4a, roughness: 0.9, metalness: 0.0 });
