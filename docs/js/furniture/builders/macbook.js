...
function buildMacBook(cfg) {
  ...
  const aluminum = makeStd({ color: 0xc0c0c8, roughness: 0.3, metalness: 0.5 });
  const dark = makeStd({ color: 0x1a1a1e, roughness: 0.7 });
  const keyMat = makeStd({ color: 0x111114, roughness: 0.8 });
  const screenGlow = new THREE.MeshBasicMaterial({ color: 0x06b6d4 });
  const trackpadMat = makeStd({ color: 0xd0d0d8, roughness: 0.3, metalness: 0.4 });
