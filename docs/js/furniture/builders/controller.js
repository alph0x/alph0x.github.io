...
function buildController(cfg) {
  ...
  const bodyMat = makeStd({ color: 0x1c1c1f, roughness: 0.5, metalness: 0.2 });
  const btnMat = makeStd({ color: 0x444444, roughness: 0.4, metalness: 0.3 });
  ...
  g.add(makeBox(makeStd({ color: 0xcc3333, roughness: 0.4 }), [0.02, 0.005, 0.02], [0.05, 0.045, 0.02]));
  g.add(makeBox(makeStd({ color: 0x3333cc, roughness: 0.4 }), [0.02, 0.005, 0.02], [-0.05, 0.045, 0.02]));
  return g;
}
