...
function buildBathroom(cfg) {
  ...
  const white = makeStd({ color: 0x3a3a45 });
  const ceramic = makeStd({ color: 0x4a4a55 });
  g.add(makeBox(white, [0.35, 0.35, 0.18], [0, 0.55, -0.2]));
  g.add(makeBox(ceramic, [0.3, 0.35, 0.38], [0, 0.4, 0.05]));
  g.add(makeBox(makeStd({ color: 0x4a4a55 }), [0.32, 0.04, 0.4], [0, 0.6, 0.05]));
  ...
  g.add(makePlane(makeStd({ color: 0x8888aa }), [0.42, 0.47], [0.5, 1.15, 0.57]));
