...
export async function buildLevel(scene: THREE.Scene, worldState: WorldState): Promise<void> {
  ...
  for (const f of ROOM_LAYOUT.furniture || []) {
    ...
    worldState.room.interactables.push({ mesh: mesh, type: f.type, panelId: f.panelId ?? (typeof result.panelId === 'string' ? result.panelId : undefined), name: f.name ?? label ?? f.type, label });
