...
export function placeDecorations(scene, worldState, decorations) {
  for (const dec of decorations) {
    if (dec.type === 'neonSign') {
      ...
      worldState.effects.particles.push({ mesh, vx: (Math.random() - 0.5) * 0.01, vy: 0.015 + Math.random() * 0.01, vz: (Math.random() - 0.5) * 0.01, life: 1.0, isSteam: true });
